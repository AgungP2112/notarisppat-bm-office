<?php if (php_check_hak_akses('pengaturan_jabatan__recycle_bin') == 'true') { ?>
    <script>
        ...
        channel.bind('pushPengaturanJabatan', function(data) {
        ...
        function loadTableMaster() {
            tableMaster = $('#tableMaster').DataTable({
                ...
                order: [],
                ajax: {
                    url: '<?= base_url('pengaturan/jabatan/load/table/recycle_bin') ?>',
                    type: "POST"
                },
                keys: true,
                select: true,
            ...
            if (jsCheckHakAkses('pengaturan_jabatan__restore') == 'true') {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?= base_url('pengaturan/jabatan/process/restore') ?>',
                            dataType: 'json',
                            data: {
                                jabatan_id: id
                            },
                ...
                alert(forbiddenAccess('pengaturan_jabatan__restore'));
            }
        };
    </script>
<?php } ?>
    ...
    <script>
        function processRestoreBatch() {
            if (jsCheckHakAkses('pengaturan_jabatan__restore_batch') == 'true') {
                ...
                if (checkBox.length > 1) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: '<?= base_url('pengaturan/jabatan/process/restore_batch') ?>',
                                dataType: 'json',
                                data: {
                                    id: dataList
                                },
                ...
                alert(forbiddenAccess('pengaturan_jabatan__restore_batch'));
            }
        };
    </script>
<?php } ?>
            ...
            if (jsCheckHakAkses('pengaturan_jabatan__destroy') == 'true') {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?= base_url('pengaturan/jabatan/process/destroy') ?>',
                            dataType: 'json',
                            data: {
                                jabatan_id: id
                            },
                ...
                alert(forbiddenAccess('pengaturan_jabatan__destroy'));
            };
        };
    </script>
<?php } ?>
    ...
    <script>
        function processDestroyBatch() {
            if (jsCheckHakAkses('pengaturan_jabatan__destroy_batch') == 'true') {
                ...
                if (checkBox.length > 1) {
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: '<?= base_url('pengaturan/jabatan/process/destroy_batch') ?>',
                                dataType: 'json',
                                data: {
                                    id: dataList
                                },
                ...
                alert(forbiddenAccess('pengaturan_jabatan__destroy_batch'));
            }
        };
    </script>
<?php } ?>